...
if [ -f "/root/reality.json" ] && [ -f "/root/sing-box" ] && [ -f "/root/public.key.b64" ] && [ -f "/etc/systemd/system/sing-box.service" ]; then
	...
	3)
			...
			;;	
        4)
        wget https://github.com/SagerNet/sing-box/releases/latest/download/${package_name}.tar.gz
...
echo ""
# Ask for server name (sni)
read -p "输入server name/SNI (默认: one-piece.com): " server_name
server_name=${server_name:-one-piece.com}
